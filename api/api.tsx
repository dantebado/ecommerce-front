...
export function addProductToCart(cartId: number | string, products : {productId:number | string, count:number}[]): Promise<Wrapper<Cart>> {
  return axios.post(API_ROOT + `/carts/${cartId}/products/`, products.map(v => {
    return {
      product: v.productId,
      count: v.count
    }
...
export function modifyProductInCart(cartId: number | string, productId: number | string, count: number): Promise<Wrapper<Cart>> {
  return axios.post(API_ROOT + `/carts/${cartId}/products/`, [{product:productId, count:count}])
}
...
export function createIndividualPurchaseFromPurchase(purchaseId: number|string, shipmentAddress:Address, clientEmail: string): Promise<Wrapper<IndividualPurchase>> {
...
export function processPayment(paymentId: string | number, payload?: any): Promise<Wrapper<Payment>> {
  return axios.put(API_ROOT + `/purchases/payments/${paymentId}/vendor`, payload)
}
